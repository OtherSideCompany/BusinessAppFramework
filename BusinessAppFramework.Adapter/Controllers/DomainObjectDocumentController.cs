...
namespace BusinessAppFramework.Adapter.Controllers
{
    ...
    public abstract class DomainObjectDocumentController<TDomainObject> : ControllerBase where TDomainObject : DomainObject
    {
        ...
        [HttpPost($"{DocumentRouteSegments.Upload}/{{{ApiRouteParams.DomainObjectId}:int}}")]
        [RequestSizeLimit(50_000_000)]
        public async Task<ActionResult<int>> UploadAsync(
            [FromRoute(Name = ApiRouteParams.DomainObjectId)] int domainObjectId,
            [FromForm] string relationKey,
        ...
        [HttpDelete($"{DocumentRouteSegments.Delete}/{{{ApiRouteParams.DomainObjectId}:int}}")]
        ...
        [HttpGet($"{DocumentRouteSegments.Exists}/{{{ApiRouteParams.DomainObjectId}:int}}")]
        public async Task<ActionResult<bool>> ExistsAsync(
            [FromRoute(Name = ApiRouteParams.DomainObjectId)] int domainObjectId, CancellationToken ct)
        {
        ...
        {
            var readResult = await _domainObjectDocumentService.OpenReadAsync(documentId, ct);
